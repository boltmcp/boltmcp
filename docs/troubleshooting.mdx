...
**Symptoms:** the browser reaches BoltMCP fine, but the web, rest-api, or seed pods fail with OIDC discovery errors against `auth.<domain>` — `ENOTFOUND`, connection timeouts, or `ECONNREFUSED 127.0.0.1` (a workstation-only `/etc/hosts` entry leaking into cluster DNS and resolving to the pod's own loopback).
...
The OIDC client secrets in the `boltmcp-oidc` Secret must match what's configured on the corresponding clients in Keycloak. There are three client secrets: `web-client-secret`, `mcp-server-client-secret` and `rest-api-to-keycloak-client-secret`. To rotate a value:
...
If importing an OpenAPI spec from the **APIs** page fails with `413 Request Entity Too Large`, the file exceeds the ingress body-size limit. The chart-managed Ingress allows 10 MB by default; raise it in your values file:
...
If you run your own Ingress manifest rather than the chart-managed one, set the same annotation there - without it NGINX applies a 1 MB default. Note that NGINX buffers the whole upload before forwarding it, so very large values increase memory and disk use on the ingress controller.
