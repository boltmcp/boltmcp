...
export const inputParamsSchema = {
  ...
  "supplierId": z.string().describe("The suppliers's id."),
  ...
  "isTpar": z.boolean().describe("If the supplier is Tpar.").optional(),
  "bankDetails": z.object({ "name": z.string().max(50).nullable().describe("The bank account's name."), "bankBranchNumber": z.string().regex(new RegExp("^[0-9]*$")).max(10).nullable().describe("The bank account's branch number."), "bankAccountNumber": z.string().regex(new RegExp("^[0-9]*$")).max(50).nullable().describe("The bank account's number.") }).strict().describe("Details of a supplier's bank account to create or update.").nullable().describe("The supplier's bank account details.").optional()
}