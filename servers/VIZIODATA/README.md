...
```json
{
  "mcpServers": {
    "VIZIODATA": {
      ...
      "url": "https://mcp.open-mcp.org/api/server/VIZIODATA@latest/mcp"
    }
  }
}
```
...
```ini
https://mcp.open-mcp.org/api/server/VIZIODATA@latest/mcp?FORWARD_VAR_OPEN_MCP_BASE_URL=https%3A%2F%2Fapi.example.com
```
...
```bash
npx @open-mcp/config add VIZIODATA \
